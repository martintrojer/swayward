...
pub struct WindowRule {
    ...
    pub sway_border: Option<SwayWindowBorderStyle>,
    #[knuffel(child, unwrap(argument))]
    pub sway_border_width: Option<u16>,
    #[knuffel(child, unwrap(argument, str))]
    pub sway_floating_border: Option<SwayWindowBorderStyle>,
    #[knuffel(child, unwrap(argument))]
...
/// A concrete border style applied when a matching window opens.
///
/// Unlike sway's `border` command, a window rule cannot use `toggle`: rules
/// describe initial state rather than an operation. `csd` remains valid because
/// sway accepts it in `for_window` commands (`sway/commands/border.c`).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SwayWindowBorderStyle {
    None,
    Normal,
    Pixel,
    Csd,
...
impl FromStr for SwayWindowBorderStyle {
...
impl From<SwayWindowBorderStyle> for BorderStyle {
    fn from(style: SwayWindowBorderStyle) -> Self {
        match style {
            SwayWindowBorderStyle::None => Self::None,
            SwayWindowBorderStyle::Normal => Self::Normal,
            SwayWindowBorderStyle::Pixel => Self::Pixel,
            SwayWindowBorderStyle::Csd => Self::Csd,
        }
    }
}

...
pub enum ConfigPath {
    ...
    Regular {
        /// User config path, usually `$XDG_CONFIG_HOME/swayward/config.kdl`.
        user_path: PathBuf,
        /// System config path, usually `/etc/swayward/config.kdl`.
        system_path: PathBuf,
    },
}
...
impl ConfigPath {
    ...
    /// The path that [`Self::load`] reads, if it exists.
    ///
    /// Returns `None` only when no candidate is present yet, in which case
    /// `load_or_create` reports the path it created.
    pub fn resolved(&self) -> Option<&Path> {
        match self {
            ConfigPath::Explicit(path) => Some(path.as_path()),
            ConfigPath::Regular {
                user_path,
                system_path,
            } => {
...
mod tests {
    ...
    fn workspace_parses_ordered_output_fallbacks() {
        let config = Config::parse_mem(
            "workspace \"web\" { sway-output-assignment \"missing\" \"HDMI-A-1\"; }",
        )
        .unwrap();
    ...
    fn retired_horizontal_view_movement_animation_is_rejected() {
        let error =
            Config::parse_mem("animations { horizontal-view-movement { duration-ms 100; }; }")
                .unwrap_err();
        assert!(
            format!("{error:?}")

...
pub struct Modeline {
    ...
    pub hsync_polarity: swayward_ipc::HSyncPolarity,
    /// Vertical sync polarity: "+vsync" or "-vsync".
    pub vsync_polarity: swayward_ipc::VSyncPolarity,
}
...
pub struct MaxBpc(pub swayward_ipc::MaxBpc);
...
impl OutputName {
    ...
    pub fn matches(&self, target: &str) -> bool {
        ...
        //
        // This is used by swayward msg output.
        if self.make.is_none() && self.model.is_none() && self.serial.is_none() {
            return false;
        }
    ...
    fn raw_decode(
        ...
        match &**value {
            knuffel::ast::Literal::Int(ref val) => match u8::try_from(val) {
                Ok(v) => swayward_ipc::MaxBpc::try_from(v)
                    .map(MaxBpc)

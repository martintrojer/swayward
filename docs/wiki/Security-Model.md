swayward assumes that programs running unsandboxed on the host are **trusted**.

This is a reasonable assumption because programs running on the host have a wide variety of ways to get all access they need, even without swayward.
For instance:
...
Anything with access to swayward's Wayland socket can, among other things:
...
- Kill a running lockscreen, create a new lock surface, and tell swayward to unlock a locked session.

Anything with access to swayward's [IPC](./IPC.md) socket can, among other things:

- Spawn a Wayland client which can do everything in the list above.

Anything with access to swayward's D-Bus interfaces can, among other things:
...
Also, while swayward doesn't directly integrate Xwayland, it's worth reminding that anything with access to the X11 `$DISPLAY` (which comes both as a socket file on disk **and** as an abstract socket in the network namespace) can intercept and emulate all input and record the contents of any X11 windows on the same `$DISPLAY` (but not Wayland windows).

## Running untrusted clients

For running untrusted clients, you need a proper sandbox that:

- Removes swayward's IPC socket.
- Prevents D-Bus access to host services.
- Uses a filtered Wayland socket.

For creating a filtered Wayland socket, you can use the [security-context](https://wayland.app/protocols/security-context-v1) protocol which swayward implements.
...
Filtering just the Wayland socket (and leaving, for example, unrestricted D-Bus access) is **not enough** to prevent untrusted clients from doing bad things.
...
The **quit** action is allowed—you can always quit swayward, even when on a lock screen.
Therefore, you must ensure that quitting swayward does not drop you into an unprotected TTY commandline.
Usually, a display manager, like GDM, will do this for you: when swayward exits (via the quit bind or if it crashes), it'll put you back into a safe password prompt.

Other than quitting, the only way to exit a lock screen is for the lock screen client to tell swayward to unlock the session.

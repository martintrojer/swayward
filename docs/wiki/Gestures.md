...
swayward supports interactive move and resize gestures for windows. A vertical
four-finger swipe opens and closes the workspace overview. A `toggle-overview`
bind provides keyboard access. You can adjust the overview's appearance in
[miscellaneous configuration](./Configuration:-Miscellaneous.md).
Swayward does not include niri's horizontal viewport gesture because the nested

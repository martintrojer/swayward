...
swayward has several options that are only useful for debugging, or are experimental and have known issues.
...
Make swayward render the monitors the same way as for a screencast or a screen capture.
...
Force-disables all outputs upon resuming swayward (TTY switch or waking up from suspend).
This causes a modeset/screen blank on all outputs.

If swayward rendering is corrupted, or monitors don't light up after a TTY switch, you can try this flag.
...
Override the DRM device that swayward will use for all rendering.

You can set this to make swayward use a different primary GPU than the default one.
...
List DRM devices that swayward will ignore.
Useful for GPU passthrough when you don't want swayward to open a certain device.
...
Make swayward create its D-Bus interfaces even if it's not running as a `--session`.

Useful for testing screencasting changes without having to relogin.

The main swayward instance will *not* currently take back the interfaces when you close the test instance, so you will need to relogin in the end to make screencasting work again.
...
This is a thing on NVIDIA proprietary drivers, so this flag can be used to test that swayward doesn't break too hard on those systems.
...
By default, swayward will disable the internal laptop monitor when the laptop lid is closed.
...
Disables the make/model/serial monitor names, as if swayward fails to read them from the EDID.
...
By default, swayward ignores xdg-activation tokens with invalid serials, to prevent windows from randomly stealing focus.
This debug flag makes swayward honor such tokens, making the aforementioned widely-used apps get focus when clicking on their tray icon or notification.

Use the [`on-xdg-activate` window rule](./Configuration:-Window-Rules.md#on-xdg-activate) to control what swayward does for individual windows when it accepts an xdg-activation request.

Amusingly, clicking on a notification sends the app a perfectly valid activation token from the notification daemon, but these apps seem to ignore it.
...
swayward drops the Activated state for unfocused windows, matching sway.

Disable this debug flag to retain niri's behavior of keeping the Activated state on unfocused workspaces and invisible tabbed windows, which can reduce unwanted animations.
...
By default, swayward will try to output a 10-bit color format to the monitor (before falling back to 8-bit).

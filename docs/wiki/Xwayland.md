...
xwayland-satellite works well with most applications: Steam, games, Discord, and Ardour with wine Windows VST plugins.
...
You can run an X11 application in [Cage](https://github.com/cage-kiosk/cage), a nested Wayland session with Xwayland support.
...
2. Run `cage -- /path/to/application`.
...
It's possible to run some games as native Wayland clients, sidestepping the issues related to X11. You can do it with a custom version of Proton like [Proton-GE](https://github.com/GloriousEggroll/proton-ge-custom) by setting the `PROTON_ENABLE_WAYLAND=1` environmental variable in the game's launch parameters. For now this is an experimental feature; it might not work with every game and might have its own issues.

...
The script caps memory and wall time, unsets `SWAYSOCK` and `I3SOCK` so the
nested compositor cannot adopt the outer one's socket, and strips
`spawn-at-startup` so a spawned bar does not compete for the IPC socket. It
rewrites `Super+` binds to `Mod+`, because the outer compositor takes `Super`
before the nested window sees it.

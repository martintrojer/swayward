...
| Rectangle roles | Set a window's `deco_rect` equal to its outer `rect` | Caught: focused test failed | The focused test compares the equality relationship between each leaf's outer and decoration rects. It does not yet require swayward's zero-height `deco_rect` to match sway's 27-pixel titlebar. This is a renderer and emitter gap: `Tile` draws an outline, and niri's configurable `TabIndicator` is an accent rather than sway's titlebar, so no matching rendered rectangle exists to serialize. |

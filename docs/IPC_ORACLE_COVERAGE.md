...
| `GET_INPUTS` | Omit keyboard `repeat_rate` | Caught: 146 passed, 1 failed | The live wire test checks the complete keyboard and pointer arrays, including identifiers, types, repeat values, XKB layout values, and keyboard-only field omission on the pointer. |
| `GET_INPUTS` conditional fields | Emit XKB layout fields on the pointer | Caught: 146 passed, 1 failed | The exact pointer object has only `identifier`, `name`, and `type`. |

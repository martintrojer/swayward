...
src/command.rs, src/layout/mod.rs, src/layout/monitor.rs, src/layout/workspace.rs, src/layout/tiling_tree/mod.rs — transfer focused container subtrees between workspaces without flattening their structure or losing internal focus history.
...
contrib/sway-to-kdl — translate title-regex assignments, preserve distinct workspace and output assignment targets, fail loudly on unsupported workspace-number assignments, and translate the supported `for_window layout tabbed, focus` subset into native window rules without altering regex escapes.

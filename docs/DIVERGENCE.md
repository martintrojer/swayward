...
src/layout/tiling_tree/mod.rs — apply split commands to an empty workspace root, expose workspace-root focus and active tile state, retain intentional nested and tabbed/stacked singleton wrappers, assign new siblings an equal share, use directional focus and resize by parent layout axis, preserve MRU focus through removal and reinsertion, preserve source wrappers while directional moves attach and then squash the completed tree, and restore the recorded split axis for `layout default`, matching sway's tree behavior.
...
contrib/sway-to-kdl — translate combined app-id/title criteria and portable border, assignment, floating, layout, focus, `no_focus`, and workspace-placement rules without altering regex escapes; fail loudly on X11-only criteria and unsupported rule actions.

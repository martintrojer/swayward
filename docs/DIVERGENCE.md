...
src/layout/mod.rs, src/layout/workspace.rs, src/layout/tile.rs, src/window/mapped.rs — expose focused-tree layout mutation and direct container focus for sway runtime commands, including sway's parent targeting, workspace-level layer history, one-level singleton flatten, focus rank and parent preservation across floating transitions, global focus-timestamp ordering after cross-output moves, and fullscreen state for floating-origin command checks.
...
swayward-ipc/src/command.rs, src/command.rs, src/layout/{mod.rs,workspace.rs,tile.rs,tiling_tree/{mod.rs,geometry.rs}}, src/ipc/tree.rs — parse and apply per-window sway border styles and widths, and expose the effective state through GET_TREE.
...
swayward-ipc/src/command.rs, src/command.rs, src/layout/mod.rs, src/layout/workspace.rs, src/layout/tiling_tree/mod.rs — move windows and container subtrees to marked destinations using sway's sibling-versus-child insertion rules while preserving subtree focus and marks; enforce global mark uniqueness across window and split-container storage; swap same-workspace tiling nodes selected by `con_id` or mark while preserving focus order and transferring fullscreen state with the occupied positions.
...
contrib/sway-to-kdl, swayward-config/src/window_rule.rs, src/window/mod.rs, src/layout/tile.rs, src/handlers/compositor.rs — translate sway's default border directives into initial tiled and floating border rules without changing swayward's shipped visual defaults; also translate combined app-id/title criteria and portable assignment, floating, layout, focus, `no_focus`, `focus_on_window_activation` urgent/focus/none, mouse-warping, and workspace-placement rules without altering regex escapes, while failing loudly on X11-only criteria, visibility-dependent activation policy, and unsupported rule actions.

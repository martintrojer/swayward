...
A gap is one of three kinds, and only one of them can break a script:

- **Config format**, 7 entries. The behaviour exists but is configured in KDL.
  `swayward-sway-to-kdl` converts these and reports what it cannot.
- **Command gap**, 1 entry. `append_layout` is the only sway command swayward
  does not implement. It returns a structured failure.
- **Deliberate**, 11 entries. Swayward follows sway where sway and i3 differ,
  so a script written against sway behaves the same.

The command families below are what the parser accepts. Anything outside them
...
| `GET_INPUTS` | Implemented | Returns live input devices. Keyboard entries include repeat and XKB layout state; pointers include sway's scalar scroll factor; physical libinput devices include USB IDs and supported live libinput properties. |

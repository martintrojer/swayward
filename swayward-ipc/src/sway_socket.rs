...
use crate::wire::{decode_header, encode, HEADER_SIZE};
...
impl SwaySocket {
    ...
    pub fn read_event(&mut self) -> Result<(u32, String), SwayError> {
        let mut header = [0u8; HEADER_SIZE];
        self.stream.read_exact(&mut header)?;
        let (msg_type, len) = decode_header(&header).map_err(SwayError::Wire)?;
        let mut body = vec![0u8; len as usize];
        self.stream.read_exact(&mut body)?;
        Ok((msg_type as u32, String::from_utf8_lossy(&body).into_owned()))
    }
}

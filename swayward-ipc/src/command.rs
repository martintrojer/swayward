...
fn parse_gaps(args: &[&str]) -> Result<Command, String> {
    let [kind, scope, operation, raw_amount] = args else {
        return Err("Expected 'gaps inner|outer|horizontal|vertical|top|right|bottom|left current|all set|plus|minus|toggle <px>'".into());
    };
    let (inner, sides) = match kind.to_ascii_lowercase().as_str() {
        "inner" => (true, [false; 4]),
        "outer" => (false, [true; 4]),
        ...
        _ => return Err("invalid gaps type".into()),
    };
    let all = match scope.to_ascii_lowercase().as_str() {
        "all" => true,
        "current" => false,
        _ => return Err("gaps scope must be current or all".into()),
    };
    let operation = match operation.to_ascii_lowercase().as_str() {
        ...
        _ => return Err("invalid gaps operation".into()),
    };
    let amount = raw_amount
        .strip_suffix("px")
        .or_else(|| raw_amount.strip_suffix("PX"))
        .unwrap_or(raw_amount)
        .parse()
        .map_err(|_| "gap amount must be an integer with optional px suffix")?;
    Ok(Command::Gaps {
        inner,
        sides,
        all,

...
| `135-floating-focus.t` | 82 | fail (60 pass) | All commands parse. `focus tiling`, `focus floating`, and `focus mode_toggle` restore the target layer's active window (`sway/commands/focus.c:273-302,418-425`). Swayward now also serializes `floating_nodes` back-to-front after raising, consistent with sway's list order (`sway/tree/container.c:1682-1693`, `sway/ipc-json.c:533-539`); no captured sway fixture contains two floating windows. Assertions 31–35 still traverse i3-only wrapper children. Nested close restoration now preserves MRU rank while a window moves through the floating layer, matching sway's persistent seat focus stack (`sway/input/seat.c:219-231,260-315`). Remaining failures cover directional floating focus, focus-child history, and i3-only tree shape. |

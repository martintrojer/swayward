...
| `219-ipc-window-focus.t` | 30 | 1 pass; 29 fail | Its workspace-output configuration translates, and the adapter now forwards upstream subtest callback arguments. The top-level focus assertion passes; all nine subtests run, but their 29 nested assertions fail because swayward emits no window events. Sway emits window `focus` events from seat focus changes (`sway/input/seat.c:1197`), `close` from container destruction (`sway/tree/container.c:494`), and scratchpad `move` events (`sway/commands/scratchpad.c:60-89`). |
...
| `285-sticky.t` | 11 | 4 pass; 7 fail | Its workspace-output configuration now translates and all assertions run. Every failure follows rejected `sticky` commands, which sway implements (`sway/commands/sticky.c:9-49`) but swayward does not yet parse. |

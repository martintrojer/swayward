...
hit-test path. It settles pending client configures first because the conformance
harness disables movement and resize animations, then applies sway's default
focus-follows-mouse behavior (`sway/config.c:272`). Of the 17 upstream files that
...
| `246-window-decoration-focus.t` | 3 | 1 pass; 2 fail | All three assertions now exercise real pointer motion and rendered-geometry hit testing with sway's default focus-follows-mouse behavior. Pointer entry focuses an ordinary decorated leaf. Tabbed and stacked decoration entry still fails because swayward ignores `HitType::Activate { is_tab_indicator: true }`, while sway focuses any newly hovered visible view (`sway/input/seatop_default.c:538-605`). |

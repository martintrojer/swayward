...
| `274-move-branch-position.t` | 16 | 10 pass; 6 fail | The standalone `splith` and `splitv` aliases now run through the same split implementation as `split h` and `split v`, so all assertions execute. The remaining six failures are the separate directional branch-position defect: sway prepends parallel right/down moves and recurses through the inactive child for perpendicular moves (`sway/commands/move.c:80-89,112-165`). |

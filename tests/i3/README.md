...
| `132-move-workspace.t` | 160 | 149 pass; 2 incidental helper skips; 9 fail | Focused and criteria-selected parent containers move as intact subtrees, preserving internal focus and matching sway's detach, attach, then source cleanup order (`sway/sway/commands.c:301-326`; `sway/commands/move.c:419-585`). The nine parent-container failures are fixed and the marked split's tree-shape assertions 157 and 159 now pass. Four floating-parent and five floating/fullscreen assertions remain for the separate floating transfer task. The two reported skips are `is_num_children` consequences when failed floating-parent moves never create their destinations, not classified conformance skips. The final command intentionally targets an empty workspace, which sway has no container for, and its exact failure is allowlisted. |

...
| `294-focus-order.t` | 61 | 47 pass; 14 fail | Assertions 1–45 pass and pin focus order through tabbed/stacked layout changes, directional moves, cross-output moves, split extraction, move-to-mark, and move-to-workspace. Three `swap container with id` commands are rejected; assertions 46–53 then fail. Sway implements `swap container with id|con_id|mark <arg>` and preserves or transfers focus according to the swapped containers (`sway/sway/commands/swap.c:13-100`). Assertions 54 and 57 pass, while 55–56 and 58–59 expose the deferred mixed floating/tiling global focus fallback; sway resolves inactive focus from one seat-wide focus stack (`sway/sway/input/seat.c:1361-1409`). The now-available `cmp_tree` helper reaches two final cases, bringing the measured total from 59 to 61; both fail and remain production focus-order findings rather than adapter gaps. |
...
| `294-focus-order.t` | 61 | 47 pass; 14 fail | The final two `cmp_tree` subtests now run, taking the file from 59 to 61 top-level assertions. Earlier mixed tiled/floating closes and rejected swap commands remain; the strict compact-tree checks expose additional focus-order and final-shape failures. Sway keeps a per-seat focus stack and performs explicit raw-focus restoration during moves (`sway/input/seat.c:1116-1128`; `sway/commands/move.c:554-608`). |
| `302-tree.t` | 15 | 11 pass; 4 fail | All comparisons run. The failures are real tree-operation disagreements in move/swap scenarios; the initial compact layouts and strict comparator assertions execute. |
| `306-move-to-parent.t` | 2 | blocked by parser after comparison setup | Both subtests run, but the multi-command chain rejects criteria after semicolons (`[con_mark=_a]`), so their final layouts fail. This is a command-chain parser gap, not a `cmp_tree` approximation. |
| `307-focus-next-prev.t` | 9 | 2 pass; 7 fail | All six `cmp_tree` scenarios and the later workspace assertions run. Focus next/prev results disagree with i3; no assertions are blocked by the adapter. |
| `308-focus_wrapping.t` | 7 reached | 3 pass; 4 fail; remainder unproven | Modern `focus_wrapping yes` now maps to swayward's exact mode, including sway's case-insensitive seven true words; the strict `cmp_tree` checks expose two cross-workspace focus failures and two nested wrapping failures. The next phase requests `no`, which swayward cannot represent, so translation stops loudly before the `no`, `force`, and `workspace` scenarios. Sway distinguishes all four modes (`sway/commands/focus_wrapping.c:6-22`; `sway/sway.5.scd:743-750`), while swayward currently exposes only `yes` and `force`. |
...
| `550-split-redundant-containers.t` | 8 | 6 pass; 2 fail | All eight comparisons run. The two failures expose singleton split/layout shape differences; no comparison is skipped or weakened. |

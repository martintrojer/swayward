...
| `135-floating-focus.t` | 82 | fail (55 pass) | All commands parse. `focus tiling`, `focus floating`, and `focus mode_toggle` restore the target layer's active window, matching `sway/commands/focus.c:273-302,418-425`. Remaining failures cover directional floating focus (`focus.c:225-271,475-485`), focus-child history, floating stacking order, i3-only floating-wrapper traversal, and nested focus restoration. |

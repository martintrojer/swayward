...
| `174-border-config.t` | 13 | 3 pass; remainder blocked by translator | Bare config proves new windows report sway's normal/2px IPC default, independently of swayward's rendered 4px default border. The next phase fails loud on deprecated `new_window 1pixel`; `new_float 1pixel` is also unsupported. Sway aliases these to `default_border` and `default_floating_border` (`sway/commands/new_window.c:5-12`; `sway/commands/new_float.c:5-12`). |
...
| `181-regress-float-border.t` | 6 | 2 pass; 4 unproven | The four size assertions depend on initial X11 client-requested geometry, which native `xdg_toplevel` cannot express. The two state assertions pass: entering fullscreen and the rejected border command leave the floating container rect unchanged. Runtime border support remains a separate parser/command gap. |
...
| `132-move-workspace.t` | 160 | pass | Focused and criteria-selected parent containers move as intact subtrees, preserving internal focus. Workspace-root moves include floating children, and fullscreen moves preserve floating origin while replacing the destination's previous fullscreen node, matching sway's detach, attach, then source cleanup order (`sway/sway/commands.c:301-326`; `sway/commands/move.c:198-238,419-585`). |
...
| `169-border-toggle.t` | 20 | 7 pass; 13 fail | All nine `border` commands are rejected because swayward has no runtime border command. Sway supports `none`, `normal`, `pixel`, `csd`, and a three-state toggle, with an optional thickness (`sway/commands/border.c:13-99`). IPC currently reports the fixed sway-compatible default metadata rather than per-window state. |
...
| `242-no-focus.t` | 6 | 2 pass; 4 blocked by translator | The first phase confirms that a second ordinary window takes focus. Config import then fails loud on `no_focus`, which sway retains and applies during view mapping while exempting the first window on a workspace (`sway/commands/no_focus.c:7-34`; `sway/tree/view.c:697-732`). |
| `251-command-criteria-focused.t` | 11 | 9 pass; 2 unproven | `__focused__` matching passes for the adapter's Wayland `app_id`, title, and workspace properties, including the no-focused-window liveness check. The instance assertion cannot distinguish X11 instance from class because the portability substitution maps both to `app_id`. The `window_role` assertion cannot set that X11-only property. Sway supports focused-value matching for both X11 properties (`sway/criteria.c:362-410`). |
| `246-window-decoration-focus.t` | 3 | unproven | The adapter's `warp_pointer` is a no-op, so the one failure and two apparent passes do not exercise pointer focus. Sway focuses a newly hovered visible view (`sway/input/seatop_default.c:538-605`). A real headless pointer-motion adapter is required before these assertions can classify compositor behavior. |
| `240-focus-on-window-activation.t` | 15 | unproven | The unchanged file stops before assertions because its X11 `_NET_ACTIVE_WINDOW` constants and event path are unavailable. Sway implements the configured `urgent`, `focus`, and `none` responses (`sway/commands/focus_on_window_activation.c:3-24`; `sway/tree/view.c:477-512`). Swayward retains niri's real xdg-activation handler (`src/handlers/mod.rs:781-866`), which the harness must drive instead of fabricating an X11 event. |
...
| `232-cmd-move-criteria.t` | 22 | 11 pass; 1 fail; 10 unproven | The `id` case exposes a swayward command gap: criteria-targeted directional move is rejected, while sway runs the move handler once per criteria match (`sway/commands.c:232-326`; `sway/commands/move.c:672-718`). Its focus-preservation assertion passes. The other ten movement assertions require X11 `window_type`; native Wayland clients cannot set that property, so their corresponding focus checks pass but movement remains unproven. |
| `243-move-to-mark.t` | 50 | blocked after 11 assertions: 6 pass; 5 fail | Swayward does not parse `move [window|container] [to] mark <mark>`, which sway implements by finding the marked destination and moving the source beside or into it (`sway/commands/move.c:419-625,956`). The unchanged file then reaches an X11 urgency client-message operation that the Wayland adapter cannot perform. A temporary urgency-neutral diagnostic reaches all 50 assertions: 25 pass and 25 fail, with later failures contaminated by the missing move command. The urgency assertion is unproven, and the workspace-mark sections are i3-only because sway permits marks only on containers (`sway/commands/mark.c:15-23`). The nested-floating assertion also uses i3's floating wrapper. |
| `274-move-branch-position.t` | 16 | blocked after 12 assertions: 4 pass; 8 fail | All setup uses the standalone `splith` and `splitv` aliases, which sway registers (`sway/commands.c:134-137`) but swayward rejects. A temporary alias-only diagnostic reaches all 16 assertions: 10 pass and 6 fail. Sway's branch insertion rules match the six failing expectations: parallel right/down moves prepend, while perpendicular moves recurse through the inactive child (`sway/commands/move.c:80-89,112-165`). |

...
| `254-move-to-output-with-criteria.t` | 16 | 9 pass; 7 fail | The first isolated run reports 9 passes and 7 failures. `fresh_workspace(output => N)` cannot select the four real outputs because swayward rejects sway's `focus output <direction|name>` command (`sway/commands/focus.c:310-352`). The final command also exposes a separate parser divergence: sway accepts `move window to output fake-1 fake-2`, resolves the first name, and ignores the rest (`sway/commands/move.c:419-425,519-525`), while swayward rejects it. |

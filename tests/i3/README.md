...
| `212-assign-urgency.t` | 3 | 2 pass; 1 fail with portability substitution | Class-to-app-id assignment works without rejected commands. Windows assigned to a visible workspace on the current or another output leave it non-urgent. A window assigned to an invisible workspace maps there without stealing focus but fails to mark the target workspace urgent. The assertion is reachable with a native Wayland client and is filed for source/capture comparison against sway's assignment-before-focus path (`sway/tree/view.c:628-665,696-732,942-945`). |

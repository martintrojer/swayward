...
| `135-floating-focus.t` | 82 | fail (56 pass) | All commands parse. `focus tiling`, `focus floating`, and `focus mode_toggle` restore the target layer's active window (`sway/commands/focus.c:273-302,418-425`). Directional focus searches floating window centers and wraps within the floating layer (`sway/commands/focus.c:225-271,475-485`), but the adapter's overlapping default positions leave assertions 23–25 ambiguous. Swayward serializes `floating_nodes` back-to-front after raising (`sway/tree/container.c:1682-1693`, `sway/ipc-json.c:533-539`); assertions 31–35 still traverse i3-only wrapper children. Remaining failures cover focus-child history, i3-only tree shape, and nested focus restoration. |

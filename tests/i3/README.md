...
| `164-kill-win-vs-client.t` | 12 | blocked after 11 assertions: 10 pass; 1 fail; 1 skip | Bare `kill` passes. Sway accepts arguments because its handler does not validate `argc`, but ignores them and closes every view in the targeted container (`sway/commands/kill.c:9-31`). Swayward rejects `kill window`, so assertion 8 exposes a parser bug. The file then stops at the unavailable X11 reconnection after `kill client`. Its final assertion expects i3's client-wide close (`i3/src/commands.c:1210-1235`), while sway ignores `client` and closes only the target container; that assertion is i3-only. |

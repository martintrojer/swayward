...
All seven i3 files using `cmp_tree` avoid X11-only client operations and belong to the 217-file protocol-portable set. Six are measurable with the current adapter: `294-focus-order.t`, `302-tree.t`, `306-move-to-parent.t`, `307-focus-next-prev.t`, `309-crash-move-parent.t`, and `550-split-redundant-containers.t`. `308-focus_wrapping.t` is portable but blocked before its generated comparisons because `focus_wrapping yes` has no exact configuration translation. The adapter builds compact `H`/`V`/`S`/`T` trees through real sway commands instead of i3-only `append_layout`, then ports upstream `verify_layout`'s token-by-token layout, name, and focused-node assertions without flattening or fabricating nodes. Floating layout strings remain fail-loud because sway omits i3's `floating_con` wrappers (`sway/sway/ipc-json.c:532-540`; `i3/src/ipc.c:619-633`). `Test::More::subtest` argument forwarding remains covered by the adapter's unprototyped wrapper.
...
| `308-focus_wrapping.t` | variable | blocked by translator | `focus_wrapping yes` still fails translation before the first generated `cmp_tree` scenario. Sway accepts the setting (`sway/commands/focus_wrapping.c:5-31`). |

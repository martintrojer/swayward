...
| `233-regress-manage-focus-unmapped.t` | 2 | blocked by translator | The translator rejects both `for_window ... kill` and `for_window ... move scratchpad` before any assertion. Sway runs `for_window` commands after the view is mapped into its container (`sway/tree/view.c:569-588,942`), so both actions need explicit map-time rule support before the focus regression is testable. |
| `272-regress-focus-assign.t` | 8 | blocked after 3 assertions: 1 pass; 2 fail | The assigned Wayland window remains on the current workspace and steals focus instead of creating `targetws`. Sway resolves assignment criteria before mapping (`sway/tree/view.c:628-665`) and computes focus only after running map-time criteria (`sway/tree/view.c:942-945`), so this is a swayward assignment/focus defect. Later assertions are contaminated by the missing destination workspace; the `window_type` half is also X11-only. |
...
| `517-regress-move-direction-ipc.t` | 6 | fail | Both subtests receive zero workspace focus events after directional moves cross outputs. Sway emits workspace focus events from focus transitions; its directional move path moves to the adjacent output and retains focus on the moved container (`sway/commands/move.c:276-299,672-744`). |
| `520-regress-focus-direction-floating.t` | 1 | blocked by translator | The translator rejects `mouse_warping none` before the assertion. Sway supports this setting (`sway/commands/mouse_warping.c:5-20`); the test otherwise uses portable directional output focus and a floating Wayland window. |

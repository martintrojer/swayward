...
| `135-floating-focus.t` | 82 | unfinished: 61 pass; 13 skip; 3 unproven; 5 fail | Assertions 31, 32, 34, 35, 37, 40, 43, 46, 54, 62, 66, 73, and 74 use i3's floating wrapper or X11-only `window` field; equivalent direct-node checks pass against swayward. Assertions 23–25 are unproven because adapter-created floating windows overlap. Layer focus modes restore the target layer's active window (`sway/commands/focus.c:273-302,418-425`), and `focus child` returns from workspace focus to the previously active layer (`sway/commands/focus.c:368-377`). The remaining five failures cover cross-workspace layer transitions and nested reinsertion. See [Floating container wrappers](../../docs/KNOWN_DEVIATIONS.md#floating-container-wrappers). Swayward serializes `floating_nodes` back-to-front after raising (`sway/tree/container.c:1682-1693`, `sway/ipc-json.c:533-539`); no captured sway fixture contains two floating windows. |

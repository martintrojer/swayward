...
| `132-move-workspace.t` | 160 | 137 pass; 2 incidental helper skips; 21 fail | The incidental X11 pointer warp is a no-op in the headless Wayland fixture. Four criteria-targeted workspace moves are rejected even though sway routes criteria targets through `cmd_move_container` (`sway/sway/commands.c:301-326`; `sway/commands/move.c:419-585`). Parent-container, floating-parent, and fullscreen moves account for the remaining failures and need remeasurement after the targeted-command gap is fixed. The two reported skips are `is_num_children` consequences when failed parent moves never create their destinations, not classified conformance skips. |
...
| `166-assign.t` | 106 | 71 pass; 3 skip; 32 fail | All assertions run after adapter lifecycle and X11-stub additions. Plain workspace assignment and `for_window ... floating enable` pass. `workspace number` and output assignments are mistranslated as literal `open-on-workspace` names, while sway records distinct assignment types and resolves them with `workspace_by_number` or `output_by_name_or_id` (`sway/commands/assign.c:31-49`; `sway/tree/view.c:631-659`). The three dock-state assertions are skipped because native Wayland clients cannot create X11 dock windows. |

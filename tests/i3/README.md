...
| `219-ipc-window-focus.t` | 30 | unproven | Its `workspace X output fake-1` configuration now maps to swayward's named-workspace `open-on-output`, matching sway's exact-name initial-output assignment (`sway/tree/workspace.c:145-162`). The file next stops at a Perl adapter gap: the local `subtest` wrapper prototype does not accept upstream's callback arguments, so no assertion runs. |

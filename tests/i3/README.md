...
| `124-move.t` | 54 | 34 pass; 15 fail; 1 skip; 4 unproven | Assertion 9 exposes a separate insertion bug after focusing a split container (`sway/tree/view.c:850-901`), which leaves assertions 10–18 starting from the wrong tree. A direct-node diagnostic with the correct starting tree proves the nested directional moves now pass, including assertion 20's independent two-split compaction sequence (`sway/commands/move.c:112-163,300-413`). Assertions 21, 25, 30, and 34 expose niri's 50 px floating step instead of sway's 10 px (`sway/commands/move.c:672-710`); assertion 37 is rejected because custom pixel distances are unimplemented; assertion 41 expects i3 percentage-point movement, while sway parses the numeric prefix and moves 25 pixels (`sway/commands/move.c:672-710`), so the percentage assertion is skipped; assertions 45–50 exercise unimplemented position forms that sway supports (`sway/commands/move.c:779-918`). Assertions 51–54 also traverse i3-only floating wrappers, so their targeting remains unproven until the position command exists. |
...
| `131-stacking-order.t` | 7 | 6 pass; 1 fail | Assertion 6 cannot focus down after `layout stacking; focus parent; split h; focus child`. Sway treats stacked parents as vertical during directional focus (`sway/commands/focus.c:158-203`) and applies `split h` to the focused parent's parent (`sway/commands/layout.c:134-183`), so the stacked child should still cycle downward. |

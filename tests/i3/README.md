...
| `132-move-workspace.t` | 160 | 138 pass; 2 incidental helper skips; 20 fail | Criteria-targeted leaf moves now use the matched window instead of the focused-window path, matching sway's per-match command context (`sway/sway/commands.c:301-326`; `sway/commands/move.c:419-585`). The two title-targeted commands pass; the two marked-split commands now reach an exact allowlisted container-subtree failure instead of the generic criteria rejection. Parent-container moves account for nine failures, floating-parent moves for four, and floating/fullscreen moves for seven. The two reported skips are `is_num_children` consequences when failed parent moves never create their destinations, not classified conformance skips. |

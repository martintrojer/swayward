...
| `517-regress-move-direction-ipc.t` | 6 | pass | A horizontal directional move at the workspace edge moves the focused container to the adjacent output and keeps it focused. Sway's resulting seat workspace transition emits one workspace `focus` event whose `current` payload is the destination workspace (`sway/commands/move.c:168-190,276-299,672-744`; `sway/input/seat.c:1098-1113,1185-1207`). The exact event sequence has not been captured from real sway; the unchanged regression independently requires one focus event and validates its destination name and window count. |

...
| `294-focus-order.t` | 61 | 47 pass; 8 unproven; 6 fail | Assertions 1–45 pass and pin focus order through tabbed/stacked layout changes, directional moves, cross-output moves, split extraction, move-to-mark, and move-to-workspace. Assertions 46–53 use `swap ... id`, whose target is an X11 window ID in sway (`sway/sway/commands/swap.c:22-31,55-58`); native Wayland windows have no such identity, so these assertions remain unproven. Swayward implements the portable `con_id` and `mark` forms with exact validation errors, preserves focus order and container marks, and transfers fullscreen state with the occupied positions. Assertions 54 and 57 pass, while 55–56 and 58–59 expose the deferred mixed floating/tiling global focus fallback; sway resolves inactive focus from one seat-wide focus stack (`sway/sway/input/seat.c:1361-1409`). The final two `cmp_tree` subtests run and fail on the same focus-order gap. |

...
| `132-move-workspace.t` | 160 | flaky: 159–160 pass | Assertion 160 intermittently fails because its final `mark a` can leave the same mark on the split container marked earlier in the file. The following `[con_mark=a] move` then moves that stale target and creates the destination workspace. This violates sway's global mark uniqueness (`sway/commands/mark.c:46-58`). An instrumented runner now names the file and failed TAP assertion. A temporary diagnostic using a fresh mark made assertion 160 pass in 10/10 isolated runs; the unchanged file failed in 2/10 isolated and 3/8 completed full-suite runs. The other assertions prove that focused and criteria-selected parent containers move as intact subtrees, preserving internal focus and fullscreen state (`sway/commands/move.c:198-238,419-585`). |
...
| `169-border-toggle.t` | 20 | finished: 18 pass; 2 skip | Runtime `border none`, `normal`, `pixel`, and the three-state toggle update rendering and GET_TREE metadata, including optional thickness, matching sway (`sway/commands/border.c:13-99`; `sway/sway/ipc-json.c:755-761`). Assertions 3–4 use i3's legacy `border 1pixel` spelling, which sway rejects; sway's equivalent is `border pixel 1`, so those assertions are skipped. Native clients without an xdg-decoration object reject `border csd`; toggle therefore cycles normal → none → pixel → normal as sway does for such clients. |

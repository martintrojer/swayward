...
`inputs.json` was captured from a capped sway 1.11 Wayland-backend session on
2026-09-20. It records the backend-neutral pointer and keyboard values.
`inputs-libinput.json` was captured the same day from a capped
`WLR_BACKENDS=headless,libinput` session using the operator's Logitech G703.
...
The `*.sequence.json` files preserve complete ordered event lists from the same
sway 1.11 installation. `workspace-switch-empty` captures a switch to
...
were captured from a capped headless sway 1.11 session on 2026-09-16. Mapping a

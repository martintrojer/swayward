...
fn nested_container_titlebars_show_the_tree_and_update_after_close() {
    for parent_layout in [Layout::Tabbed, Layout::Stacked] {
        ...
        assert_eq!(
            geometry.titlebars[&outer_first].title,
            "V[window 2 T[window 3 window 4]]"
        );

        t.remove_tile_node(inner_second);
        assert_eq!(
            t.compute_geometry().titlebars[&outer_first].title,
            "V[window 2 window 3]"
        );
    }
}
...
#[test]
fn tab_strips_square_only_the_top_window_corners() {
...
#[test]
fn tab_strips_square_only_the_rows_that_touch_them() {
    // Inside a tab, a vertical split stacks rows: only the TOP row meets the
    // strip. The lower rows sit further down the window and must keep their
    // rounded top corners. A horizontal split is the opposite case - every
    // column reaches the strip, so they all square.
    let configured = swayward_config::CornerRadius {
        top_left: 11.,
        top_right: 12.,
        bottom_right: 13.,
    ...
    let squared = swayward_config::CornerRadius {
        top_left: 0.,
        top_right: 0.,
        ..configured
    };
    ...
    let add = |t: &mut TilingTree<TestWindow>, id: usize| {
        ...
        t.add_tile(
            Tile::new(window, size, 1., clock, Rc::new(Options::default())),
            InsertTarget::Focused,
        )
    };

    for (split, top_is_squared, rest_is_squared) in
        [(Layout::SplitV, true, false), (Layout::SplitH, true, true)]
    {
        let mut t = tree((1200., 800.), 0.);
        ...
        assert_eq!(
            radius_of(&t, first),
            if top_is_squared { squared } else { configured },
            "{split:?}: first child"
        );
        assert_eq!(
            radius_of(&t, second),
            if rest_is_squared { squared } else { configured },
            "{split:?}: second child"
        );
    }
}

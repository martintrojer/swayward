...
impl<W: LayoutElement> TilingTree<W> {
    ...
    pub fn update_render_elements(&mut self, is_active: bool, layer: RenderLayer) {
        ...
        for (id, node) in &mut self.nodes {
            ...
            tile.set_titlebar_attached(geometries.titlebar_attached.contains(id));
    ...
    pub fn render<R: NiriRenderer>(
        ...
        for (id, titlebar) in &geometries.titlebars {
            ...
            if titlebar.visible {
                // Match the window's own top corners, so a tab bar does not
                // draw square shoulders above a rounded frame.
                let radius = self
                    .tile(*id)
                    .map(|tile| tile.window().geometry_corner_radius())
                    .unwrap_or_default();

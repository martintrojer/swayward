...
pub(super) enum InsertPosition {
    ...
    /// Drop landed on the middle of the tile at this index, which sway treats as
    /// a swap rather than an insertion.
    ///
    /// `seatop_move_tiling.c:365-388` sets `swap` when the target edge is
    /// `WLR_EDGE_NONE` on a container, and then calls `container_swap`.
    SwapWith(usize),
    Floating,
}
...
impl<W: LayoutElement> Monitor<W> {
    ...
    ///
    /// A workspace with a stored number reports it. An unnamed placeholder
    /// reports its position among NUMBERED workspaces rather than its raw
    /// index, because a raw index changes when an unrelated high-numbered
    /// workspace is inserted ahead of it, and a client then sees the workspace
    /// it calls "1" renumbered behind that one.
    pub fn sway_workspace_number(&self, index: usize) -> i32 {
        ...
        // Report the lowest number no other workspace already owns. Using the
        // position among numbered workspaces collided: with "1:first" present,
        // the placeholder also claimed 1, and `workspace number 1` then had two
        // candidates. Sway never hits this because it destroys the empty
        // placeholder once a real workspace exists.
        let taken = self
            .workspaces
            .iter()
            .enumerate()
    ...
    ///
    /// An unnamed placeholder is named after the number it reports, so its name
    /// cannot collide with a real workspace like "1:first" that already owns
    /// that digit prefix. `sway_display_name` alone derived the name from the
    /// index, which did collide, and `workspace number 1` then had two
    /// candidates.
    ...
    pub fn sort_sway_workspaces(&mut self) {
        ...
        // Sort by the number a client actually sees, which for an unnamed
        // placeholder is its position among numbered workspaces rather than
        // nothing at all. Keying only on the stored number sent the placeholder
        // to the end, so inserting workspace 93 put it ahead of the workspace
        // clients call 1. Sway orders numeric ahead of non-numeric and sorts
        // stably (sway/sway/tree/output.c:387-405).
        // Include the current position in the key so equal-ranked workspaces
        // keep their creation order, which is what sway's list_stable_sort
        // guarantees. Without it, a workspace inserted before the trailing
        // placeholder overtook an equal-ranked one created earlier.
        let keys = (0..self.workspaces.len())
            .map(|index| {
                let workspace = &self.workspaces[index];
                // Tie-break on the workspace id, which is a monotonic
                // creation counter, rather than the current position. Sway's
                // list_stable_sort keeps equal-ranked workspaces in creation
                // order, and our insert index places a new workspace before the
                // trailing placeholder, so position had already inverted them.
                // The unnamed placeholder must stay LAST whatever else happens:
                // the layout relies on it as a trailing empty workspace, which
                // Monitor::verify_invariants asserts and which a drop onto an
                // empty workspace needs. Ranking it by its displayed number put
                // it ahead of a non-numeric name like "ws5", and a later move
                // then filled the workspace that had become last.
                // Rank on survival, not addressability. A workspace holding
                // only a number is addressable but disposable, so it is a
                // placeholder and must rank last; ranking it 0 for having a
                // number put it ahead of a named workspace and left the named
                // one trailing, which the invariants reject.
    ...
    pub fn consider_destroy_workspace(&mut self, id: WorkspaceId) {
        ...
        if idx == self.active_workspace_idx || self.workspaces[idx].has_windows() {
            return;
        }
        // Keep the trailing placeholder, which the layout relies on as a drop
        // target for an interactive move onto an empty workspace. The test is
        // purely positional: the last workspace IS the placeholder, whether or
        // not it currently carries a number. Requiring it to be unaddressable
        // dropped that protection as soon as initialize_workspaces_from_bindings
        // handed it a number, and the monitor was then left ending in a
    ...
    pub fn attach_workspace(&mut self, mut ws: Workspace<W>, mut idx: usize, activate: bool) {
        ...
        let needs_empty_workspace = idx == self.workspaces.len() - 1 && ws.has_windows();
        self.workspaces.insert(idx, ws);
        if needs_empty_workspace {
            self.add_workspace_bottom();
        }
        ...
        if self.workspace_switch.is_none() {
                ...
                .skip(1)
            {
                if idx != self.active_workspace_idx {
                    ...
                    assert!(
                        ws.has_windows() || ws.has_sway_identity(),

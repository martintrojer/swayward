...
impl<W: LayoutElement> Workspace<W> {
    ...
    ///
    /// `index` is only a fallback for a workspace with no stored identity: such
    /// a placeholder reports its 1-based position. Callers that can supply the
    /// position among NUMBERED workspaces should do so, because a raw index
    /// changes when an unrelated high-numbered workspace is inserted ahead of
    /// it, and a client then sees the workspace it calls "1" renumbered.
    ...
    ///
    /// Deliberately *not* [`Workspace::has_sway_identity`]. Addressable and
    /// durable are different questions, and conflating them is what made the
    /// placeholder invariants unfixable for five attempts. A workspace that
    ...
    pub fn update_config(&mut self, base_options: Rc<Options>) {
        ...
        self.working_area = apply_outer_gaps(
            output_area,
            options.layout.outer_gaps,
            options.layout.gaps,
            options.layout.outer_gaps_configured,
    ...
    pub fn add_tile_to_column(
        ...
        let target = self
            ...
            .iter_depth_first()
            .filter_map(|(id, node)| {
                matches!(node, crate::layout::tiling_tree::TreeNode::Leaf { .. }).then_some(id)
            })
            .nth(col_idx)
            .map(InsertTarget::Node)
            .unwrap_or(InsertTarget::Focused);
    ...
    pub(super) fn scrolling_insert_position(&self, pos: Point<f64, Logical>) -> InsertPosition {
        let leaves = self
            ...
            .enumerate()
            .collect::<Vec<_>>();
        // Sway splits a tile into edge regions and a centre: an edge inserts
        // before or after, the centre SWAPS (seatop_move_tiling.c:365-388,
        // where swap is WLR_EDGE_NONE on a container). Use the outer quarter of
        // each tile as its edges, leaving the middle half as the swap region.
        if let Some((index, (tile, tile_pos, _))) = leaves
            .iter()
            .find(|(_, (tile, tile_pos, _))| tile_pos.x + tile.tile_size().w > pos.x)
        {
            let width = tile.tile_size().w;
            let edge = width / 4.0;
            if pos.x < tile_pos.x + edge {
                InsertPosition::NewColumn(*index)
            } else if pos.x > tile_pos.x + width - edge {
                InsertPosition::NewColumn(index + 1)
            } else {
                InsertPosition::SwapWith(*index)
            }
        } else {
            InsertPosition::NewColumn(leaves.len())
        }
    }
    ...
    pub(super) fn insert_hint_area(
        ...
        match position {
            ...
            InsertPosition::SwapWith(index) => {
                // The hint covers the whole target tile: the drop exchanges the
            ...
            InsertPosition::InColumn(_, _) | InsertPosition::Floating => None,
        }
    }

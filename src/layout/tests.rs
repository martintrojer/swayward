...
#[test]
fn move_to_named_target_index_keeps_placeholder_last() {
    // Fuzzer seed: a numbered-but-unnamed workspace is addressable, so the
    // placeholder logic keeps it, while the invariant asked whether it
    // survives. The two must ask the same question.
    check_ops([
        Op::AddNamedWorkspace {
            ws_name: 1,
            output_name: None,
...
#[test]
fn unname_then_implicit_rename_keeps_placeholder_last() {
    // CI shrank `random_operations_dont_panic` to this six-op sequence, which
    // trips the "non-active workspace can't be empty and unnamed except the
    // last one" assert at monitor.rs:2268. Both the move and the rename use
    // the implicit target (`None`), and the same output is added twice.
    check_ops([
        Op::UnnameWorkspace { ws_name: 1 },
        Op::AddOutput(1),
        Op::AddOutput(1),
...
fn drag_over_creation_slot_uses_preview_then_materializes_workspace() {
    ...
    let creation_slot = monitor.workspaces.last().unwrap().id();
    let creation_slot_index = monitor.workspaces.len() - 1;
    let creation_slot_geo = monitor.workspaces_render_geo().last().unwrap();
    let pointer = creation_slot_geo.loc + creation_slot_geo.size.to_point().downscale(2.);
    ...
    assert_eq!(preview.insertion_index, creation_slot_index);
    assert_eq!(preview.geometry, creation_slot_geo);
    assert_eq!(preview_geo, creation_slot_geo);
    ...
    let (_, _, destination) = layout
        ...
        .find(|(_, _, workspace)| workspace.has_window(&0))
        .unwrap();
    assert_ne!(destination.id(), creation_slot);
...
fn drop_on_a_tile_centre_across_outputs_exchanges_windows() {
    ...
    let target_workspace = layout
        ...
        .active_workspace_ref()
        .id();
    let (target_pos, target_size) = layout
        .monitor_for_output(&outputs[1])
        .unwrap()
        .active_workspace_ref()
...
fn drop_on_a_tile_centre_swaps_instead_of_inserting() {
    ...
    assert!(
        matches!(
            workspace.scrolling_insert_position(centre),
            crate::layout::monitor::InsertPosition::SwapWith(_)
        ),
        "a centre drop must swap"
    );
    assert!(
        matches!(
            workspace.scrolling_insert_position(left_edge),
            crate::layout::monitor::InsertPosition::NewColumn(_)
        ),

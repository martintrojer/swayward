...
impl MoveGrab {
    pub fn new(
        ...
        Self::with_threshold(state, start_data, window, enable_view_offset, move_icon, 8.)
    }

    /// Creates a tiled move that stays pending until `recognition_threshold` is exceeded,
    /// matching sway's strict squared-distance check (`sway/input/seatop_move_tiling.c:38-59`).
    pub fn new_tiling(
        state: &mut State,
        start_data: AnyStartData<State>,
        window: Window,
    ...
    fn with_threshold(
        ...
        let (output, pos_within_output) = state.swayward.output_under(location)?;
    ...
    fn begin_move(&mut self, data: &mut State) -> bool {
        if !data.swayward.layout.interactive_move_begin(
            self.window.clone(),
            &self.start_output,
    ...
    fn on_frame(&mut self, data: &mut State) -> bool {
        ...
        if self.gesture == GestureState::Recognizing {
            ...
            if c.x * c.x + c.y * c.y > self.recognition_threshold * self.recognition_threshold {
        ...
        match self.gesture {
            ...
            GestureState::Move => {
                ...
                let ongoing = data.swayward.layout.interactive_move_update(
                    &self.window,
                    delta,
                    output,
                    pos_within_output,
            ...
            GestureState::ViewOffset => {
                let res = data.swayward.layout.view_offset_gesture_update(
                    -relative_delta.x,
                    timestamp,
                    false,
                );

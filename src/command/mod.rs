...
fn execute_one(
    ...
    let action = match parsed.command {
        ...
        Command::Resize {
            ...
            match axis {
                ...
                direction => {
                    ...
                    if state.swayward.layout.resize_window_edge(None, edge, change) == Some(false) {
                        return failure("Cannot resize any further");
                    }
                    None
                }
            }
        ...
        Command::CreateOutput => {
            ...
            let crate::backend::Backend::Headless(headless) = backend else {
                return failure("Expected a multi backend");
            };
            if let Err(error) = headless.create_output(swayward) {
                return failure(error);
            }
            None
        }

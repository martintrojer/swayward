...
/// caller, so assert it against the source.
#[test]
fn no_test_server_adopts_the_ambient_swaysock() {
    ...
    let adopting_callers = source.matches(needle.as_str()).count();
    assert_eq!(
        adopting_callers, 0,
...
fn get_inputs_and_seats_return_sway_schema_and_values() {
    let config = swayward_config::Config::parse_mem(
        r#"input { keyboard { repeat-delay 321; repeat-rate 42; xkb { layout "us,ru"; }; }; }"#,
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        serde_json::json!([
            {
                "identifier": "0:0:test_keyboard",
                "name": "test keyboard",
                "type": "keyboard",
                "repeat_delay": 321,
                "repeat_rate": 42,
                "xkb_layout_names": ["English (US)", "Russian"],
                "xkb_active_layout_index": 0,
                "xkb_active_layout_name": "English (US)"
            },
            {
                "identifier": "0:0:test_pointer",
                "name": "test pointer",
                "type": "pointer"
            }
        ])
...
fn initially_floating_window_uses_and_clamps_client_size() {
    ...
    assert_eq!(clamped["geometry"]["width"], 1280);
    assert_eq!(clamped["geometry"]["height"], 800);

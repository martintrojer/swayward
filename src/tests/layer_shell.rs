...
fn floating_windows_do_not_overlap_exclusive_layers() {
    ...
    for (edge, anchor, size, position) in cases {
        ...
        let clears_bar = match edge {
            "top" => pos.y >= 30.,
            "bottom" => pos.y + size.h <= 570.,
            "left" => pos.x >= 40.,
            "right" => pos.x + size.w <= 760.,
            _ => unreachable!(),
        };
        assert!(
            clears_bar,
            "floating window overlaps the {edge} bar at {pos:?} with size {size:?}"
        );

        if edge == "top" {
            f.swayward().layout.move_floating_window(
                None,
                swayward_ipc::PositionChange::SetFixed(-10_000.),
                swayward_ipc::PositionChange::SetFixed(0.),
                false,
            );
            let workspace = f.swayward().layout.active_workspace().unwrap();
            let (_, pos, _) = workspace.tiles_with_render_positions().next().unwrap();
            assert!(pos.x < 0., "physical-edge off-screen allowance was removed");
        }
    }
}

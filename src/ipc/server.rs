    ...
    loop {
        ...
        let event = match input {
            ...
            StreamInput::Header(header) => {
                ...
                let msg_type = MessageType::Subscribe;
                let requested: Vec<String> = match serde_json::from_slice(&payload) {
                    Ok(requested) => requested,
                    // sway replies `{"success": false}` to an unparseable
                    // subscribe and keeps the client
                    // (`sway/sway/ipc-server.c:733-741`).
                    Err(_) => {
                        queue_ipc_message(
                            &mut write_buffer,
                            &mut write_buffer_size,
        ...
        let (msg_type, payload) = match event {
            ...
            Event::WorkspaceUrgencyChanged { current } if subscriptions.contains("workspace") => (
                1 << 31,
